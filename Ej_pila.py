...
def vuelta_robot(p):
    ...
    if (p.direccion == "sur"):
        ...
        inv = "sur"
    elif (p.direccion == "este"):
        inv = "oeste"
    elif (p.direccion == "oeste"):
        inv = "este"
    elif (p.direccion == "sureste"):
        inv = "noroeste"
    elif (p.direccion == "noreste"):
        inv = "suroeste"
    elif (p.direccion == "suroeste"):
        inv = "noreste"
    elif (p.direccion == "noroeste"):
        inv = "sureste"

while (tamanio(pila_robot) < 8) :
    r = robot()
    r.pasos = int(input("Cantidad de pasos: "))
    r.direccion = input("Direccion: ")
    apilar(pila_robot,r)

print('Vuelta del robot hacia el lugar de donde partio: ')
while (not pila_vacia(pila_robot)):
    p = desapilar(pila_robot)
    vuelta_robot(p)
    print(p.pasos,'pasos al',inv)
"""
...
#******************************************************************************************************************************
"""
21. Dada una pila con los valores promedio de temperatura ambiente de cada día del mes de  abril, obtener 
la siguiente información sin perder los datos: 
...
while tamanio(pila) < 5:
    temp = int(input("Ingrese la temperatura del dia de hoy: "))
    apilar(pila,temp)
    media += temp

minimo,maximo = temp,temp
media = media // (cima(pila)+1)
...
print("El mes de abril tuvo una minima de ",minimo,"° y una maxima de ",maximo,"°")
print("La media de temperatura fue: ",media,"°")
print("Teniendo",debajo,"dias por debajo y ",encima," dias por encima de la media ")
...
while(not pila_vacia(pila_personajes)):
    ...
    if(personaje[0][0] in ['C', 'D', 'G']):
        print('el personaje',personaje[0],'comienza con:', personaje[0][0])
    i += 1 
"""